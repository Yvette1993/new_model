...
def parse_command():
    ...
    model_names = ['NewModel','NewModelSkipAdd','NewModelSkipConcat']  
    loss_names = ['l1', 'l2']                       
    ...
    import argparse
    parser = argparse.ArgumentParser(description='NewModel_depth')
     # lisa-> train_args
    parser.add_argument('--arch', '-a', metavar='ARCH', default='NewModel', choices=model_names,          
                        help='model architecture: ' + ' | '.join(model_names) + ' (default: NewModel)')
    ...
    parser.add_argument('--decoder', '-d', metavar='DECODER', default='upconv', choices=decoder_names,
                        help='decoder: ' + ' | '.join(decoder_names) + ' (default: upconv)')
